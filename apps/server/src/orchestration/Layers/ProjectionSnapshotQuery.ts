...
function mapThreadHierarchyFields(row: Schema.Schema.Type<typeof ProjectionThreadDbRowSchema>) {
  return {
    parentThreadId: row.parentThreadId,
    origin: row.origin,
    rootThreadId: row.rootThreadId,
    depth: row.threadDepth,
  };
}
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        worktreePath: threadRow.value.worktreePath,
        ...mapThreadHierarchyFields(threadRow.value),
